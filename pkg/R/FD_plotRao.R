...
FD_plotRao <- function(df, save.plot=TRUE, ext=".png"){
  pacman::p_load(ggplot2,viridis, rlist, zoo)
  plotsRao <- list()
  ...
  a <- ggplot(data = norm_summary1, aes(x=as.numeric(as.character(Time)), y = value, colour = variable, group= variable)) +
    geom_point() +
    geom_line(aes(y=rollmean(value, 3, na.pad=TRUE))) +
  ...
  b <- ggplot(data = norm_summary1, aes(x=as.numeric(as.character(Time)), y = norm_value, colour = variable, group= variable)) +
    geom_point() +
    geom_line(aes(y=rollmean(norm_value, 3, na.pad=TRUE))) +
