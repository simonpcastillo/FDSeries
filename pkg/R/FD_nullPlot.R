...
FD_nullPlot <- function(d.rao, d.FD, sumRao, sumFD, plotRao=TRUE, saveRao=TRUE, plotFRic=TRUE,saveFRic=TRUE){
  pacman::p_load(ggplot2,viridis,gridExtra, grid,rlist,zoo)
  ...
  if(plotRao==TRUE){
    ...
    RaoSummary <- sumRao
    n1<-ggplot(RaoSummary, aes(as.numeric(as.character(Time)), as.numeric(as.character(Simpson)), group=1))+
      geom_point()+
      geom_line() +
      scale_x_continuous(limits = c(-280, -160), breaks = seq(-280, -160, 20))+
      labs(x="Time", y = " Simpson index")+
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mSimpson), group=1), colour="blue") +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mSimpson + Simpson.ic95), group=1), colour="blue", lty=2) +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mSimpson - Simpson.ic95), group=1), colour="blue", lty=2) +
      theme_bw()+
      theme(axis.text.x = element_text(angle=90))


    n2<-ggplot(RaoSummary, aes(as.numeric(as.character(Time)), as.numeric(as.character(FunRao)), group=1))+
      geom_point()+
      geom_line() +
      labs(x="Time", y = " Rao index")+
      scale_x_continuous(limits = c(-280, -160), breaks = seq(-280, -160, 20))+
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRao), group=1), colour="blue") +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRao + FunRao.ic95), group=1), colour="blue", lty=2) +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRao - FunRao.ic95), group=1), colour="blue", lty=2) +
      theme_bw()+
      theme(axis.text.x = element_text(angle=90))

    n3<-ggplot(RaoSummary, aes(as.numeric(as.character(Time)), as.numeric(as.character(FunRedundancy)), group=1))+
      geom_point()+
      geom_line() +
      scale_x_continuous(limits = c(-280, -160), breaks = seq(-280, -160, 20))+
      labs(x="Time", y = " Redundancy index")+
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRedun), group=1), colour="blue") +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRedun + FunRed.ic95), group=1), colour="blue", lty=2) +
      geom_line(data=d.rao, aes(x=as.numeric(as.character(Time)), y=(mFunRedun - FunRed.ic95), group=1), colour="blue", lty=2) +
      theme_bw()+
      theme(axis.text.x = element_text(angle=90))
  ...
  if(plotFRic==TRUE){
    ...
    FDSummary <- sumFD
  n4<-ggplot(FDSummary, aes(as.numeric(as.character(time)), as.numeric(as.character(FRic)), group=1))+
    geom_point()+
    geom_line() +
    scale_x_continuous(limits = c(-280, -160), breaks = seq(-280, -160, 20))+
    geom_line(data=d.FD, aes(x=as.numeric(as.character(Time)), y=(mFRic), group=1), colour="blue") +
    geom_line(data=d.FD, aes(x=as.numeric(as.character(Time)), y=(mFRic + FRic.ic95), group=1), colour="blue", lty=2) +
    geom_line(data=d.FD, aes(x=as.numeric(as.character(Time)), y=(mFRic - FRic.ic95), group=1), colour="blue", lty=2) +
